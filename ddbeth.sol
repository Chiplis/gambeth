...
contract WeiStakingByDecentralizedDegenerates is usingProvable {
    ...
    event CreatedBet(string indexed, string, uint64);
    event PlacedBet(string indexed, string indexed, string, string, uint256);
    ...
    function createBet(string calldata betId, string calldata query, uint64 deadline, uint64 schedule, uint256 commission, uint256 minimum, string calldata description) public payable {
        ...
        emit CreatedBet(betId, betId, schedule);
    }
    ...
    function placeBets(string calldata betId, string[] calldata results, uint256[] calldata amounts) public payable {
        ...
        for (uint i = 0; i < results.length; i++) {
            ...
            require(bytes(results[i]).length > 0 && total >= value && value >= betMinimums[betId]);
            total -= value;
            
            // Update all required state and emit the event for the frontend
            resultPools[betId][results[i]] += value;
            userPools[betId][msg.sender] += value;
            betPools[betId] += value;
            userBets[betId][msg.sender][results[i]] += value;
            emit PlacedBet(betId, results[i], betId, results[i], value);
        }
        if (total != 0) {
