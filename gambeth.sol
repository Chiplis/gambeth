...
contract Gambeth is usingProvable {
    ...
    function createOracleBet(string memory betType, string memory betId, string calldata query, uint64 deadline, uint64 schedule, uint256 commission, uint256 minimum, uint256 initialPool, string calldata description) public payable {
    ...
    function decideHumanBet(string memory betId, string memory result) public payable {
        require(
            ...
            && msg.sender == betOwners[betId]
            && betSchedules[betId] <= block.timestamp
            && !finishedBets[betId],
            "Unable to decide bet, check arguments"
        );
